// Jira Cloud's v3 comment API requires the comment `body` in Atlassian Document Format (ADF), a
// structured document tree — a plain string is rejected. This builds the minimal ADF document
// for a caller-supplied plain-text comment: one paragraph, with each input line joined by an
// explicit `hardBreak` node rather than a literal "\n" in a text node (ADF text nodes don't
// render embedded newlines).
...
export function toAdfDocument(comment: string): AdfDocument {
